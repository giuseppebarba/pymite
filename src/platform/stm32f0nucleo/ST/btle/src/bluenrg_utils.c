...
static uint32_t updater_calc_crc(const uint8_t * data, uint16_t nr_of_bytes)
{
	...
	for (i = 0; i < nr_of_bytes; i += 4) {
		value =
		    data[i] | ((uint32_t) data[i + 1] << 8) | ((uint32_t)
							       data[i +
								    2] << 16) |
		   ...
		   uint8_t skip_bootloader_verif)
{
	...
	if (version < SUPPORTED_BOOTLOADER_VERSION_MIN
	    || version > SUPPORTED_BOOTLOADER_VERSION_MAX)
  ...
  ************************************************************************/
	...
	for (i = FW_OFFSET; i < FULL_STACK_SIZE; i += SECTOR_SIZE) {
  ...
  ************************************************************************/
	...
	for (i = FW_OFFSET; i < (number_sectors * SECTOR_SIZE);
	     i += DATA_SIZE) {
		...
		ret =
		    aci_updater_program_data_block(BASE_ADDRESS + i, DATA_SIZE,
						   addr_image);
		if (ret)
			return BLE_UTIL_ACI_ERROR;
		HCI_Process();
	...
	if (fw_size % SECTOR_SIZE) {
		...
		if (skip_bootloader_verif) {
			last_image =
			    fw_image + (number_sectors - 1) * SECTOR_SIZE;
			last_size =
			    fw_size - (number_sectors - 1) * SECTOR_SIZE;
		} else {
		...
		for (i = 0; i < SECTOR_SIZE; i += DATA_SIZE) {
			ret =
			    aci_updater_program_data_block(address + i,
							   DATA_SIZE,
							   last_sector + i);
			if (ret)
				return BLE_UTIL_ACI_ERROR;
  ...
  ************************************************************************/
	...
	for (i = (verify_from_sector * SECTOR_SIZE);
	     i < (number_sectors * SECTOR_SIZE); i += SECTOR_SIZE) {
		...
		if (skip_bootloader_verif) {
			crc2 =
			    updater_calc_crc(fw_image + i - FW_OFFSET,
					     SECTOR_SIZE);
		} else {
...
int read_IFR(uint8_t * data)
{
  ...
  ************************************************************************/
	...
	for (i = 66 * 1024 - 3 * 64; i < 66 * 1024; i += 64) {
...
int program_IFR(const IFR_config_TypeDef * ifr_image)
{
   ...
   * READ IFR data
   ************************************************************************/
	for (i = 0; i < (2 * 1024); i += 64) {
		ret =
		    aci_updater_read_data_block(IFR_BASE_ADDRESS + i, 64,
						ifr_data + i);
  ...
  ************************************************************************/
	...
	for (i = 0; i < (2 * 1024); i += 64) {
		ret =
		    aci_updater_program_data_block(IFR_BASE_ADDRESS + i, 64,
						   ifr_data + i);
		if (ret != BLE_STATUS_SUCCESS)
			return BLE_UTIL_ACI_ERROR;
	}
  ...
  ************************************************************************/
	{
		...
		for (i = 0; i < (2 * 1024); i += 64) {
			ret =
			    aci_updater_read_data_block(IFR_BASE_ADDRESS + i,
							64, ifr_updated);
			if (ret != BLE_STATUS_SUCCESS) {
				return ret;
			}
...
uint8_t verify_IFR(const IFR_config_TypeDef * ifr_data)
{
	...
	for (i = 0; i < (3 * 64); i += 64) {
		ret =
		    aci_updater_read_data_block((IFR_BASE_ADDRESS + 2048 -
						 IFR_SIZE) + i, 64,
						ifr_updated);
		if (ret != BLE_STATUS_SUCCESS) {
			return ret;
...
uint8_t getBlueNRGVersion(uint8_t * hwVersion, uint16_t * fwVersion)
{
	...
	status =
	    hci_le_read_local_version(&hci_version, &hci_revision,
				      &lmp_pal_version, &manufacturer_name,
				      &lmp_pal_subversion);
