...
tBleStatus aci_gap_init(uint8_t role, uint16_t * service_handle,
...
tBleStatus aci_gap_set_discoverable(uint8_t AdvType, uint16_t AdvIntervMin,
				    uint16_t AdvIntervMax, uint8_t OwnAddrType,
				    uint8_t AdvFilterPolicy,
				    uint8_t LocalNameLen, const char *LocalName,
				    uint8_t ServiceUUIDLen,
				    uint8_t * ServiceUUIDList,
				    uint16_t SlaveConnIntervMin,
				    uint16_t SlaveConnIntervMax)
...
tBleStatus aci_gap_set_direct_connectable(uint8_t own_addr_type,
...
tBleStatus aci_gap_slave_security_request(uint16_t conn_handle, uint8_t bonding,
...
tBleStatus aci_gap_start_auto_conn_establishment(uint16_t scanInterval,
						 uint16_t scanWindow,
