...
typedef struct _gap_set_direct_conectable_cp {
...
typedef struct _gap_set_non_connectable_cp {
...
typedef struct _gatt_find_info_req_cp {
	uint16_t conn_handle;
...
typedef struct _l2cap_conn_param_update_resp_cp {
	uint16_t conn_handle;
	uint16_t interval_min;
	uint16_t interval_max;
	uint16_t slave_latency;
