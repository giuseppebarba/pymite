...
int hci_le_set_advertising_parameters(uint16_t min_interval,
				      ...
				      tBDAddr direct_bdaddr, uint8_t chan_map,
				      uint8_t filter);
...
int hci_le_encrypt(uint8_t key[16], uint8_t plaintextData[16],
...
int hci_le_create_connection(uint16_t interval, uint16_t window,
			     ...
			     const tBDAddr peer_bdaddr, uint8_t own_bdaddr_type,
			     uint16_t min_interval, uint16_t max_interval,
			     uint16_t latency, uint16_t supervision_timeout,
			     uint16_t min_ce_length, uint16_t max_ce_length);

int hci_read_transmit_power_level(uint16_t * conn_handle, uint8_t type,
				  int8_t * tx_level);
...
int hci_le_read_local_version(uint8_t * hci_version, uint16_t * hci_revision,
